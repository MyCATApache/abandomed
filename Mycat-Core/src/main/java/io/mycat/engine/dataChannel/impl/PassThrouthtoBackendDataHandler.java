...
public class PassThrouthtoBackendDataHandler implements DataHandler {
    ...
    public void transfer(MySQLConnection in, boolean isTransferLastPacket, boolean transferFinish, boolean isAllFinish) throws IOException {
        ...
        backendCoxn.getProtocolStateMachine().setNextState(frontCoxn.getProtocolStateMachine().getNextState());
        backendCoxn.getNetworkStateMachine().setNextState(WriteWaitingState.INSTANCE);
        backendCoxn.getNetworkStateMachine().driveState();
        ...
        backendCoxn.setWriteCompleteListener(() -> {
            ...
            if (transferFinish) { //透传全部完成
                backendCoxn.getNetworkStateMachine().setNextState(ReadWaitingState.INSTANCE);
            } else {
                backendCoxn.getNetworkStateMachine().setNextState(NoReadAndWriteState.INSTANCE);
                frontCoxn.getNetworkStateMachine().setNextState(ReadWaitingState.INSTANCE);
                frontCoxn.getNetworkStateMachine().driveState();
            }
